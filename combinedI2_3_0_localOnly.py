...
if (runMode == "y")|(runMode == "Y"):
    ...
    video_file_path = askopenfilename(initialdir=os.getcwd(),filetypes=[('video files', '.MOV'),('video files', '.mp4'),('all files', '.*')])
    localFlag=True
...
#""""""""""""""""""""""""""""""""file processing begins after this
    if localFlag:
        file_path=video_file_path
        dateName=strftime("%Y_%m_%d_%H:%M", time.localtime())
    else:
        file_path=fileName
    ...
    if RecordFlag:
        if localFlag==False:
            ...
            outFileName=file_path[:-3]+'Processed.mp4'
        if iTimeLapseFlag:
    ...
    if localFlag==False:
        ...
        outExcelFileName=file_path[:-3]+'Data.xlsx'
